...
const Svg = () => {
  const [color, setColor] = useState("fill-amber-500");

  const randColor = () => {
    const cols = ["fill-amber-500", "fill-gray-200"];
    const randcolor = Math.floor(Math.random() * cols.length);
    setColor(cols[randcolor]);
    console.log(cols[randcolor]);
  };

  const customClass = `${color} transition ease-in-out hover:-translate-y-1 hover:scale-101 delay-150 duration-300 bg-none m-1`;
  return (
    <div className={customClass}>
      <svg
        onMouseEnter={() => randColor()}
        xmlns="http://www.w3.org/2000/svg"
        version="1.0"
        viewBox="0 0 891.000000 436.000000"
